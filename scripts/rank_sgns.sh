...
function usage {
    ...
    echo ""
    echo "  Usage:" 
    echo "      rank_sgns.sh <id> <window_size> <dim> <k> <s> <min_count1> <min_count2> <itera> [-s]" 
    echo ""
...
outdir=output/${id}/ranking/${identifier}
resdir=results/${id}/ranking/${identifier}
...
if [ $# -eq 8 ]
    then 
        # Clean directory
        rm -r output/${id}/ranking/${identifier}
fi